// Simplified Tailwind v3 color palette
...
function euclideanDistance(color1: { r: number; g: number; b: number }, color2: { r: number; g: number; b: number }): number {
  return Math.sqrt(
...
export function findNearestTailwindColor(hex: string): string {
  ...
  for (const [colorName, shades] of Object.entries(tailwindColors)) {
    for (let i = 0; i < shades.length; i++) {
      const shadeColor = hexToRgb(shades[i]);
      const distance = euclideanDistance(targetColor, shadeColor);
      
      if (distance < minDistance) {
        minDistance = distance;
        closestColor = `${colorName}-${(i + 1) * 100}`;
      }
    }
  }
